...
class Coeff(object):
    def __init__(self,deltaT,maxX,kinetics,mode,zeta,K0,Kf,Kb,alpha,Lambda,asymParameter,gamma,dA,dB,dC,dY,dZ,mechanism,outerBoundaryMode):
        self.n = 0
        self.xi = 0.0
        self.maxX = maxX
    ...
    def calc_jacob(self,x,Theta):
        ...
        if self.kinetics == 'Nernst':
            ...
            #Nernst Condition
            self.J[0,1] = -((self.dA*np.exp(Theta)+self.dB)/h)
            self.J[0,5] = self.dA / h
            self.J[0,6] = self.dB / h
            self.J[1,0] = -((self.dB/np.exp(Theta)+self.dA)/h)
            self.J[1,5] = self.dA / h
            self.J[1,6] = self.dB  / h
        ...
        for row in range(5,self.n*5-5,5):
            ...
            if self.mechanism ==0:
                ...
                self.J[row+1,row+1] += self.Kf                
                self.J[row+1,row+2] += -self.Kb
                self.J[row+2,row+1] += - self.Kf
                self.J[row+2,row+2] += self.Kb
            elif self.mechanism == 3:
                self.J[row+1,row+1] += 2.0*self.Kf*x[5*i+1]
                self.J[row+1,row+2] += -2*self.Kb
                self.J[row+2,row+1] += -0.5*-2.0*self.Kf * x[5*i+1]
                self.J[row+2,row+2] += self.Kb
            elif self.mechanism == 5:
                self.J[row,row+3] += -self.Kf
                self.J[row,row] += self.Kb 
                self.J[row+3,row+3] += self.Kf
                self.J[row+3,row] += -self.Kb
            elif self.mechanism == 6:
                self.J[row,row] += self.Kb*x[5*i+2]
                self.J[row,row+2] += self.Kb*x[5*i]
                self.J[row,row+3] += -self.Kf
                self.J[row+2,row] += self.Kb*x[5*i+2]
                self.J[row+2,row+2] +=self.Kb*x[5*i]
                self.J[row+2,row+3] += -self.Kf
                self.J[row+3,row] += -self.Kb*x[5*i+2]
                self.J[row+3,row+2] += -self.Kb*x[5*i]
                self.J[row+3,row+3] += self.Kf
            else:
                raise ValueError
    ...
    def calc_fx(self,x,Theta):
        ...
        for j in range(5,5*self.n-5,5):
            ...
            if self.mechanism ==0:
                ...
                self.fx[j+1] += self.Kf*x[5*i+1] - self.Kb*x[5*i+2]
                self.fx[j+2] += - self.Kf*x[5*i+1] + self.Kb*x[5*i+2]
            elif self.mechanism == 3:
                self.fx[j+1] += self.Kf*x[5*i+1]*x[5*i+1] - 2.0*self.Kb*x[5*i+2]
                self.fx[j+2] +=  -0.5*self.Kf*x[5*i+1]*x[5*i+1] + self.Kb*x[5*i+2]
            elif self.mechanism == 5:
                self.fx[j] += -self.Kf*x[5*i+3] + self.Kb*x[5*i]
                self.fx[j+3] +=  +self.Kf*x[5*i+3] - self.Kb*x[5*i]
            elif self.mechanism == 6:
                self.fx[j] += -self.Kf*x[5*i+3] + self.Kb*x[5*i]*x[5*i+2]
                self.fx[j+2] += -self.Kf*x[5*i+3] + self.Kb*x[5*i]*x[5*i+2]
                self.fx[j+3] += self.Kf*x[5*i+3] - self.Kb*x[5*i]*x[5*i+2]
            else:
                raise ValueError
