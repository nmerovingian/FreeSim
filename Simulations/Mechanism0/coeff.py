...
class Coeff(object):
    def __init__(self,deltaT,maxX,K0,alpha,gamma,dA,dB,dC,dY,dZ):
        self.n = 0
        self.xi = 0.0
        self.maxX = maxX
        self.K0 = K0
    ...
    def Ccal_abc_radial(self,deltaT,Theta,deltaX):
        ...
        for i in range(1,self.n-1):
            deltaX_m = self.XX[i] - self.XX[i - 1]
            deltaX_p = self.XX[i + 1] - self.XX[i]
            self.aB[i] =self.dC*( (-(2.0 * deltaT) / (deltaX_m * (deltaX_m + deltaX_p)) + 2.0 / self.XX[i] * (deltaT / (deltaX_m + deltaX_p))))
            self.bB[i] =self.dC*( ((2.0 * deltaT) / (deltaX_p * (deltaX_m + deltaX_p)) + (2.0 * deltaT) / (deltaX_m * (deltaX_m + deltaX_p)))) + 1.0
            self.cB[i] =self.dC*((-(2.0 * deltaT) / (deltaX_p * (deltaX_m + deltaX_p)) - 2.0 / self.XX[i] * (deltaT / (deltaX_m + deltaX_p))))
